...
const Filtered = () => {
 const { searchfilter:{text ,category},
 allProducts,
 upadatefilter}=useGlobalfilter();
...
const getuniquedata=(data,properties)=>{
 ...
 const categoryData=getuniquedata(allProducts,"category");
  ...
  return (
    ...
    <input type="text" placeholder='search' name="text" value={text} onChange={upadatefilter} />
    </form>
    </div>
   ...
   type="button" name='category' value={curElem} onClick={upadatefilter} >
    <p>{curElem}</p>
   </button>
    })}
    </div>
